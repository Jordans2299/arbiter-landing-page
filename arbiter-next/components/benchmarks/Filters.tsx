...
export default function Filters({ filters, setFilters, sort, setSort, models, devices }: {
  ...
  return <section className="benchmark-controls" aria-label="Leaderboard filters">
    ...
    <div className="leaderboard-toolbar">
      ...
      <label className="toolbar-format"><span className="sr-only">Model format</span><select value={filters.format} onChange={(event) => update("format", event.target.value)}><option value="">All formats</option>{unique(models.map((model) => model.model.format)).map((value) => <option key={value}>{value}</option>)}</select></label>
      <label className="toolbar-model"><span className="sr-only">Specific model</span><select value={filters.modelKey} onChange={(event) => update("modelKey", event.target.value)}><option value="">All models</option>{models.map((model) => <option key={model.modelKey} value={model.modelKey}>{model.model.displayName}</option>)}</select></label>
      <label className="toolbar-sort"><span className="sr-only">Sort leaderboard</span><select value={sort} onChange={(event) => setSort(event.target.value as BenchmarkSort)}>
    ...
    <details className="more-filters">
      ...
      <div className="more-filter-grid">
        <label className="mobile-only-filter">Format<select aria-label="Mobile model format" value={filters.format} onChange={(event) => update("format", event.target.value)}><option value="">All formats</option>{unique(models.map((model) => model.model.format)).map((value) => <option key={value}>{value}</option>)}</select></label>
        <label className="mobile-only-filter">Model<select aria-label="Mobile specific model" value={filters.modelKey} onChange={(event) => update("modelKey", event.target.value)}><option value="">All models</option>{models.map((model) => <option key={model.modelKey} value={model.modelKey}>{model.model.displayName}</option>)}</select></label>
