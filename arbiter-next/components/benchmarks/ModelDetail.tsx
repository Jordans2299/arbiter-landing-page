  ...
  return <>
    ...
    <section className="model-detail-hero">
      <div className="model-detail-intro"><span className="eyebrow">Model benchmark</span><h1>{model.model.displayName}</h1>
        ...
        <dl className="model-facts"><Item term="Format" value={model.model.format} /><Item term="Family" value={model.model.family || "—"} /><Item term="Parameters" value={model.model.parameterSize || "—"} /><Item term="File size" value={formatBytes(model.model.fileSizeBytes)} /><Item term="Suite" value={model.suiteVersion} /></dl>
...
export function RunAccordion({ run, expanded, onToggle }: { run: BenchmarkRun; expanded: boolean; onToggle: () => void }) {
  ...
  return <article className={`run-accordion ${expanded ? "open" : ""}`}>
    ...
    {expanded && <div id={panelId} className="run-panel">
      ...
      <div className="run-detail-columns"><div><h3>Apple hardware</h3><dl className="detail-list"><Item term="Platform" value={run.device.platform} /><Item term="Hardware model" value={formatHardwareModel(run.device.hardwareModel)} /><Item term="OS version" value={run.device.osVersion || "—"} /><Item term="Physical memory" value={formatBytes(run.device.physicalMemoryBytes)} /><Item term="Processor count" value={run.device.processorCount?.toString() ?? "—"} /><Item term="Active processor count" value={run.device.activeProcessorCount?.toString() ?? "—"} /><Item term="GPU name" value={run.device.gpuName || "—"} /><Item term="Low Power Mode" value={run.device.lowPowerModeEnabled == null ? "—" : run.device.lowPowerModeEnabled ? "Enabled" : "Disabled"} /></dl></div>
